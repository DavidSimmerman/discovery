...
export const GET: RequestHandler = async ({ locals }) => {
  ...
  // libraryCount feeds the live "Shuffle N songs" total for the library source.
  const [settings, counted] = await Promise.all([
    withUserSessionLock(userId, (tx) => loadUserSettings(tx, userId)),
    db.select({ n: count() }).from(ratings).where(eq(ratings.userId, userId)),
  ]);
  return json({ settings, libraryCount: counted[0]?.n ?? 0 });
};
